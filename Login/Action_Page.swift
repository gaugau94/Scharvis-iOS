...
class Action_Page: UIViewController {
    ...
    func getItems()
    {
        let myUrl = NSURL(string : "http://192.168.1.3:4567/room?id=\(idsa)")
    ...
    func switchValueDidChange(sender:UISwitch!)
    {
        ...
        if (sender.on == true){
            print("on")
            print(sender.tag)
            let myUrl1 = NSURL(string : "http://192.168.1.3:4567/action")
            print(myUrl1)
            var stringPost = "{"
            stringPost += "\""
            stringPost += "user"
            ...
            let url:NSURL = NSURL(string:"http://192.168.1.3:4567/action")!
    ...
    {
        let myUrl = NSURL(string : "http://192.168.1.3:4567/status?id=\(idStatus)")
        let request = NSMutableURLRequest(URL:myUrl!);
        request.HTTPMethod = "GET";
        let task = NSURLSession.sharedSession().dataTaskWithRequest(request)
        {
